...
class SYSFSPin:
  def __init__(self, board=None, pin=None, mode=None):
    ...
    self.pin = pin
    self.mode = mode
    self.export_path = self.board.res["pin"]["export_path"]
    self.value_path = self.board.res["pin"]["value_path"]+str(self.pin)+'/value'
    self.direction_path = self.board.res["pin"]["value_path"]+str(self.pin)+'/direction'
    if not os.path.exists(self.board.res["pin"]["value_path"]+str(self.pin)):
...
class PWM:
  def __init__(self, board=None, pin_obj=None):
    ...
    if pin_obj.pin not in board.res["pwm"]["pinpwm"]:
      raise ValueError("%d pin is not supported by pwm%d"%pin_obj.pin,"The supported pins are",board.res["pwm"]["pinpwm"])
    self.freq_value = 100
    self.duty_value = 50
...
class Board:
  def __init__(self, boardname="", port=None):
    ...
    if platform.platform().find("rockchip") > 0:  
      self.boardname = "UNIHIKER"    #unihiker修改后讨论
    
    find_board(self)   #match自动识别
