...
class LeR():
    ...
    def quintet_intialization(self, kwargs_snr_function):  
        ...
        self.snr_calculator_dict = quintet_param_dict
        return snr_
    ...
    def lensed_rate(self, size=False, snr_threshold=8., num_img=2, jsonFile=True, none_as_nan=True):
        ...
        total_rate_step = c0 * np.mean(snr_hit*lensed_param['weights'])
        print("total unlensed rate with step function: {}".format(total_rate_step))

        # with pdet 
        total_rate_pdet = c0 * np.mean(pdet_combined*lensed_param['weights'])
        print("total unlensed rate with pdet function: {}".format(total_rate_pdet))
    ...
    def rate_comparision(self, size=False, snr_threshold=8., num_img=2, jsonFile=True, none_as_nan=True):
        ...
        rate_ratio = lensed_rate/unlensed_rate
        print('lensed/unlensed rate ratio = ', rate_ratio)
