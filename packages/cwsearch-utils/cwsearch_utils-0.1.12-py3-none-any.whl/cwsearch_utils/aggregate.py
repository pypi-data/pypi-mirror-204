...
def resources_for_tag(infinstor_time_spec, bucket, prefix, tag):
    import boto3
    import botocore
    ...
    tagkey = tag[:ind]
    tagval = tag[ind+1:]
    print(f"resources_for_tag: bucket={bucket}, prefix={prefix}, tag={tag} tagkey={tagkey} tagval={tagval} infinstor_time_spec in={infinstor_time_spec} Using {infinstor_time_spec[17:]}")
...
def get_files_list(infinstor_time_spec, s3client, bucket, prefix, log_groups, tag):
    ...
    if log_groups:
        ...
        resources = resources_for_tag(infinstor_time_spec, bucket, prefix, tag)
        if resources:
...
def populate_names(bucket, prefix, infinstor_time_spec, log_groups, tag):
    ...
    try:
        ...
        files = get_files_list(infinstor_time_spec, s3client, bucket, prefix, log_groups, tag)
    except Exception as ex:
        print(f'Caught {ex} while list_objects_v2 of {bucket} prefix {prefix} time {infinstor_time_spec}', flush=True)
        return ner_entities

    if not files:
        print(f'No files found for bucket={bucket}, prefix={prefix}, infinstor_time_spec={infinstor_time_spec}, log_groups={log_groups}, tag={tag}')
        return ner_entities

    print(f"Reverse Chronological Order Files:")
    for fl in files:
        print(f"{fl[0]}", flush=True)
