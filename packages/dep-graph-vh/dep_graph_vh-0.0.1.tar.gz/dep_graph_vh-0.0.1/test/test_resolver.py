...
class TestResolver(TestCase):
    def test_resolving_valid_graph(self):
        ...
        with open(converted_string_path) as file:
            correct_string = file.read()

        graph = resolver.get_graph(valid_graph_path)

        self.assertTrue(graph)
        self.assertTrue(type(graph) is resolver.DependencyGraph)
    ...
    def test_resolving_graph_with_cyclic_deps(self):
        ...
        """
        graph_with_cyclic_deps_path = os.path.join(CURRENT_DIR, 'test_data', 'graph_with_cyclic_deps.json')
    ...
    def test_resolving_graph_with_missing_deps(self):
        ...
        """
        graph_with_cyclic_deps_path = os.path.join(CURRENT_DIR, 'test_data', 'graph_with_missing_deps.json')
