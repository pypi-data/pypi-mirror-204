...
def update_service() -> bool:
    cf.shell('systemctl --user daemon-reload')
    cf.shell('systemctl --user enable knivesout.service')
    cf.shell('systemctl --user restart knivesout.service')
    cf.info('service restarted')
    cf.info('check status by `systemctl --user status knivesout.service`')
    return True
