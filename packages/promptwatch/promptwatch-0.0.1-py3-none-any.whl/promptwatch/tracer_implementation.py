...
class PromptWatch():
    ...
    def __init__(self, session_id:Optional[str]=None, tracking_project:Optional[str]=None, tracking_tenant:Optional[str]=None, api_key:Optional[str]=None):
        ...
        self.callback_handler=LangChainCallbackHandler(session_id=self.session_id, api_key=api_key, tracking_project=tracking_project,tracking_tenant=tracking_tenant  )
        self.callback_manager=get_callback_manager()
        
    @classmethod
    def get_active_instance(cls)->PromptWatch:
    ...
    def __enter__(self):
        ...
        self.callback_manager.set_handler(self.callback_handler)
        return self

    def __exit__(self, exc_type, exc_val, exc_tb):
        self.callback_handler.end_session()
        self.callback_manager.remove_handler(self.callback_handler)
        del self._thread_local.active_instance

    @classmethod
    def get_active_session(cls):
        return cls.get_active_instance().callback_handler.current_session

    @classmethod
    def log(cls, text:str, metadata:Optional[Dict[str,Any]]=None):
        
        cls.log_activity(Log(cls.get_active_session(), text=text, metadata=metadata))
        
    @classmethod
    def log_activity(cls, activity:ActivityBase):
        instance = PromptWatch.get_active_instance()
        if not instance:
            raise Exception("Invalid operation: you must enter an session before logging")
        instance.callback_handler._add_activity(activity)
...
class LangChainCallbackHandler(BaseCallbackHandler, ABC):
    ...
    def __init__(self, 
        ...
        if session_id:
            ...
            self.current_session = Session(id=session_id or str(uuid4()), tracking_project=tracking_project, tracking_tenant=tracking_tenant)
    ...
    def on_llm_start(
        ...
        if self.current_llm_chain:
            if self.current_llm_chain.llm and self.current_llm_chain.llm.dict:
                llm_info = self.current_llm_chain.llm.dict()
...
def _build_wrapped_function(tracer:LangChainCallbackHandler, instance,  handler_key:str, original_function):
    import types
