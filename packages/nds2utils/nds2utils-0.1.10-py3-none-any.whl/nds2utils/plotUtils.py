...
from .dataUtils import linear_log_ASD, good_ticks
...
def plot_ASDs(
    ...
    for ii, chan in enumerate(channels):
        ...
        if logbin:
            ...
            logf_high = ff[-1]
            fflog = np.logspace(np.log10(logf_low), np.log10(logf_high), num_points)
            plot_ff, plot_asd = linear_log_ASD(fflog, ff, plot_asd)
        else:
            plot_ff = ff
...
def compare_ASDs(
    ...
    for ii, chan in enumerate(channels):
        ...
        if logbin:
            ...
            logf_high = ff1[-1]
            fflog = np.logspace(np.log10(logf_low), np.log10(logf_high), num_points)
            plot_ff1, plot_asd1 = linear_log_ASD(fflog, ff1, plot_asd1)
            plot_ff2, plot_asd2 = linear_log_ASD(fflog, ff2, plot_asd2)
        else:
            plot_ff1 = ff1
            plot_ff2 = ff2
...
def plot_TFs_A(
    ...
    for ii, chanB in enumerate(channels):
        ...
        s2.semilogx(
            plotff, 180 / np.pi * np.angle(plotTF), ls=ls, alpha=alpha, label=full_label
        )
        s3.loglog(plotff, plotcoh, ls=ls, alpha=alpha, label=full_label)
...
def plot_raw_data(
    dataDict, seconds=1, downsample=1, title="Raw Data", units="cts", fig=None
):
    """
