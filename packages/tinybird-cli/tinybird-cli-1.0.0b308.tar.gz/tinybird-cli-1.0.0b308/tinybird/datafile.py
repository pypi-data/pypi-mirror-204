...
async def process_file(
    ...
    if '.datasource' in filename:
        ...
        if 'import_service' in node or 'import_connection_name' in node:
            ...
            # Substitute the known import_ parameters with the ones used by the
            # import API:
            # - If an import_ parameter is not present and there's a default
            ...
            for original, replacement, default in IMPORT_REPLACEMENTS:
                if original not in params:
                    continue
                p_value = params.get(original, default)
                del params[original]
                if p_value is not None:
                    params[replacement] = p_value
