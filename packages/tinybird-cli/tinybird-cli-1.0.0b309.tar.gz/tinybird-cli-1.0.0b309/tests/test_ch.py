...
class TestExplainEstimate(unittest.IsolatedAsyncioTestCase):
    ...
    async def create_table_and_insert_chunk(self, table_name, table_definition, chunk):
        ...
        self.client.query_sync(f"INSERT INTO {table_name} VALUES ({chunk})", read_only=False)
    ...
    async def test_explain_estimate_returns_sum_of_rows(self):
        ...
        await self.create_table_and_insert_chunk(
            first_table_name,
            f"CREATE TABLE {first_table_name} ON CLUSTER tinybird (example String) Engine=MergeTree() PARTITION BY example ORDER BY tuple()",
            "'test_string'"
        )

        await self.create_table_and_insert_chunk(
            second_table_name,
            f"CREATE TABLE {second_table_name} ON CLUSTER tinybird (example String) Engine=MergeTree() PARTITION BY example ORDER BY tuple()",
            "'test_string'"
        )
