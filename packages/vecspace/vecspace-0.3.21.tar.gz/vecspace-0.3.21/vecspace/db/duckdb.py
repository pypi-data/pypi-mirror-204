from vecspace.api.types import Documents, Embeddings, IDs, Metadatas
from vecspace.db.clickhouse import (
    Clickhouse,
...
class DuckDB(Clickhouse):
    ...
    def __del__(self):
        logger.info("Exiting: Cleaning up .vecspace directory")
        self.reset_indexes()

    def persist(self):
        raise NotImplementedError(
            "Set vecspace_impl='duckdb+parquet' to get persistence functionality"
        )
...
class PersistentDuckDB(DuckDB):
    ...
    def __init__(self, settings):
        ...
        if settings.persist_directory == ".vecspace":
            raise ValueError(
                "You cannot use vecspace's cache directory .vecspace/, please set a different directory"
            )
    ...
    def persist(self):
        ...
        self._conn.execute(
            ...
            TO '{self._save_folder}/vecspace-embeddings.parquet'
                (FORMAT PARQUET);
        """
        )
        ...
        self._conn.execute(
            ...
            TO '{self._save_folder}/vecspace-collections.parquet'
                (FORMAT PARQUET);
        """
        )
    ...
    def load(self):
        ...
        if not os.path.exists(f"{self._save_folder}/vecspace-embeddings.parquet"):
            logger.info(f"No existing DB found in {self._save_folder}, skipping load")
        else:
            path = self._save_folder + "/vecspace-embeddings.parquet"
        ...
        if not os.path.exists(f"{self._save_folder}/vecspace-collections.parquet"):
            logger.info(f"No existing DB found in {self._save_folder}, skipping load")
        else:
            path = self._save_folder + "/vecspace-collections.parquet"
