...
class Coco(BaseLabelme):
    ...
    def self2coco(self):
        ...
        for dataset in tqdm(self.datasets):
            ...
            if dataset.get('background'):  # 为真表示为处理背景图片
                ...
                # 标签属性处理
                self.labelme_shapes2coco_ann(dataset)
                # 2、添加coco类别
                print(f'coco转换数据处理开始')
                for ann in tqdm(self.one_img_ann_list):
                    category_id = 0
    ...
    def labelme_shapes2coco_ann(self, dataset):
        ...
        :return:
        """
        self.rebuild_labelme_info(dataset.get('labelme_info').get('shapes'))  # 把group_id相同的标注进行列表隔离
        for structure_shape_key, structure_shape_value in self.structure_shapes.items():
            if not structure_shape_key and structure_shape_value != 0:  # 处理不打组的数据
                self.no_group_id_shape(structure_shape_value, dataset)
            else:  # 处理打组的数据
                self.group_id_shape(structure_shape_value, dataset)
    ...
    def rebuild_labelme_info(self, labelme_info):
        """
        ...
        for shape in labelme_info:
            self.structure_shapes[shape['group_id']].append(shape)

    def no_group_id_shape(self, structure_shape_value, dataset):
        """
        处理不划分group_id组的标注shape元素列表
        :param structure_shape_value:
        :param dataset:
        """
        for shape in structure_shape_value:
            if shape.setdefault('shape_type') == 'polygon':
                self.polygon_shape(shape, dataset)
            if shape.setdefault('shape_type') == 'rectangle':
                self.rectangle_shape(shape, dataset)
            if shape.setdefault('shape_type') == 'point':
                self.point_shape(shape, dataset)
        # if len(structure_shape_value) > 4:  # 检查标注数据，针对一张图像上只有4个点的标注
    ...
    def group_id_shape(self, structure_shape_value, dataset):
        """
    ...
    def polygon_shape(self, shape, dataset):
        """
        处理标注形状为多边形的shape
        :param shape: 标注属性字典
        ...
        if self.group_id_mark:  # 处理打组的shape
            pass
        else:  # 处理不打组的shape，目前阶段多边形标注打组与不打组处理逻辑都相同
            ...
            if len(shape['points']) == 4:
                ...
                # 内存里更新shape标注属性
                self.rebuild_polygon_shape.update({'points': bbox})  # 更新多边形点的坐标为矩形框坐标
                self.rebuild_polygon_shape.update({'label': shape['label']})
                self.rebuild_polygon_shape.update({'segmentation': convert_segmentation})  # 新增关键点列表，按照左上、右上、右下、左下的顺序排序
                self.one_img_ann_list.append(self.rebuild_polygon_shape)  # 把标注shape追加到转coco时的迭代列表集合中
            else:
                print(f'车牌多边形标注不为4个点，请矫正数据，已经存储到自定义默认错误数据集目录，修改完成后覆盖真实数据集，并重新转换')
                dataset.update({'output_dir': dataset.get('error_path')})
                self.error_dataset.append(dataset)  # 把错误的图像标注存放到列表中，统一保存后方便修改
                print(dataset.get('full_path'))

    def rectangle_shape(self, shape, dataset):
        """
        处理标注形状为矩形的shape
        :param shape: 标注属性字典
        :param dataset:一张图像属性封装字典集合
        ...
        if self.group_id_mark:  # 处理打组的shape
            ...
            if shape.get('label') in self.point_label:
                print(f'点标注的标签名称有错，请核对数据，已经存储到自定义默认错误数据集目录，修改完成后覆盖真实数据集，并重新转换')
                dataset.update({'output_dir': dataset.get('error_path')})
                self.error_dataset.append(dataset)
                print(dataset.get('full_path'))
            segmentation = self.find_rect_sequential_coordinates(shape, dataset.get('full_path'))
            self.rebuild_rectangle_shape.update({'points': shape['points']})  # 更新多边形点的坐标为矩形框坐标
            self.rebuild_rectangle_shape.update({'label': shape['label']})
            self.rebuild_rectangle_shape.update({'segmentation': segmentation})  # 新增关键点列表，按照左上、右上、右下、左下的顺序排序
            self.one_img_ann_list.append(shape)  # 矩形框可以不做处理，直接追加标注

    def point_shape(self, shape, dataset):
        """
        处理标注形状为点的shape
        :param shape: 标注属性字典
        :param dataset: 一张图像属性封装字典集合
