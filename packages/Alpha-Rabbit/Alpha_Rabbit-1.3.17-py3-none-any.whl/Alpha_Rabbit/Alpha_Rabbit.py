...
class multi_factor_test(object):
    ...
    def factors_abnormal_ret(self,factordf,return_col,factorlist,pricedf = None):
        df = factordf.copy()
        if pricedf is not None:
            # 默认明收除今收
            df[return_col] = pricedf.pct_change(fill_method = None).shift(-1).stack()
...
class Barra_factor_ana(object):
    ...
    def barra_style_pool(self,style):
        ...
        bystyle[style+'_group'] = bystyle[style].dropna().groupby(level = 0,group_keys=False).apply(lambda x: pd.cut(x,10,labels=list(range(1,10+1))))
        return bystyle
    ...
    def factor_performance_bystyle(self,factordata,factorname,style):
        ...
        bystyle[style+'_group'] = bystyle[style].dropna().groupby(level = 0).apply(lambda x: pd.cut(x,10,labels=list(range(1,10+1))))
