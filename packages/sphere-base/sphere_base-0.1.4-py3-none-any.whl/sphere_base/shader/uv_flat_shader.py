...
class FlatShader(BaseShader):
    ...
    def draw(self, object_index=0, object_type="", mesh_index=0, indices_len=0, position: list = None, orientation=None,
             scale=None, texture_id=0, color=None, switch=0, line_width=1):

        scale = [scale[0] * .97, scale[1] * .97, scale[2]]
        super().draw(object_index, object_type, mesh_index, indices, vertices, position, orientation, scale, texture_id,
                     color)
