...
class RemindMail:
    ...
    def generate(self, force: bool = False, dry_run: bool = False,
            ...
            if is_match:
                ...
                if not is_command:
                    if generate_type == GenerateType.EMAIL:
                        RemindMail().send_email(item.split(' ', 1)[1],
                ...
                if token_after == 1:
                    ...
                    if not dry_run:
                        ...
                        RemindMail().log_msg(f"(in test mode- not deleting {item})",
                                             level="debug")
                elif token_after > 1:
                ...
                if is_command:
                    if not dry_run:
                        ...
                        RemindMail().log_msg(f"(in test mode- not executing {item})",
                                             level="debug")
        ...
        if not dry_run:
            ...
            RemindMail().log_msg(
                f"In test mode- would set remindmail -> day_generated to {today_index}",
                level="debug"
            )
