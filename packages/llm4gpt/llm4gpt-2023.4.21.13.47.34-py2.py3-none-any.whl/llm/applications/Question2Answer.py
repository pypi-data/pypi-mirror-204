...
class Question2Answer(object):
    ...
    @clear_cuda_cache
    def kbqa(self, query, max_turns=1, topk=3, return_source_documents=True):

        v = self.query_embedd(query)
        context = self.docs.find(v, topk=topk)[0].texts  # [:, ('text', 'scores__cosine__value')]

        query = self.prompt_template.format(context=context, question=query)

        if return_source_documents:
            return context
        return self.qa(query, max_turns)

    def search4qa(self, max_turns=1):
        from meutils.request_utils.crawler import Crawler
        url = "https://top.baidu.com/board?tab=realtime"

        _ = Crawler(url).xpath('//*[@id="sanRoot"]/main/div[2]/div/div[2]/div[*]/div[2]/a/div[1]//text()')
        return self.qa(_, max_turns)

    def qa(self, query, max_turns=1):
