...
class QA(object):
    def __init__(self, chatllm: ChatLLM, document_prompt: PromptTemplate = None):
        """
        ...
        self.retriever = self.source_service.vector_store.as_retriever(search_kwargs={"k": top_k}),  # todo
    ...
    def get_knowledge_based_answer(self, query, max_turns=3, top_k=4, **kwargs):
        ...
        llm_chain = RetrievalQA.from_llm(
            llm=self.chatllm,
            retriever=self.source_service.vector_store.as_retriever(search_kwargs={"k": top_k}),  # todo: 重复实例化优化
