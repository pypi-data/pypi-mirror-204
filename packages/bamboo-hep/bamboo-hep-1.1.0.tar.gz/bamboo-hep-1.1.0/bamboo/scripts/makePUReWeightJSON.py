...
def main():
    ...
    parser = argparse.ArgumentParser(description=(
        "Produce a BinnedValues-JSON file for pileup reweighting, "
        "using data pileup distributions obtained with "
        "`pileupCalc.py -i analysis-lumi-json.txt --inputLumiJSON pileup-json.txt "
        "--calcMode true --minBiasXsec MBXSECINNB --maxPileupBin NMAX --numPileupBins N outname.root` "
        "(see also https://twiki.cern.ch/twiki/bin/viewauth/CMS/PileupJSONFileforData#Pileup_JSON_Files_For_Run_II)"))  # noqa: 501
    parser.add_argument("-o", "--output", default="puweights.json", type=str, help="Output file name")
    parser.add_argument("-f", "--format", type=str,
                        choices=["correctionlib", "cp3-llbb"], default="cp3-llbb",
                        help="Output JSON format")
    parser.add_argument("--name", type=str, default="puweights", help=(
        "Name of the correction inside the CorrectionSet "
        "(only used for the correctionlib format)"))
    parser.add_argument("--mcprofile", help=(
        "Pileup profile used to generate the MC sample "
        "(use --listmcprofiles to see the list of defined profiles)"))
    parser.add_argument("--listmcprofiles", action="store_true",
                        help="list the available MC pileup profiles")
    parser.add_argument("--nominal", type=str, help=(
        "File with the data (true) pileup distribution histogram "
        "assuming the nominal minimum bias cross-section value"))
    parser.add_argument("--up", type=str, help=(
        "File with the data (true) pileup distribution histogram "
        "assuming the nominal+1sigma minimum bias cross-section value"))
    parser.add_argument("--down", type=str, help=(
        "File with the data (true) pileup distribution histogram "
        "assuming the nominal-1sigma minimum bias cross-section value"))
    ...
    parser.add_argument("mcfiles", nargs="*", help=(
        "MC ROOT files to extract a pileup profile from (if used)"))
    parser.add_argument("--mctreename", type=str, default="Events",
                        help="Name of the tree to use in mcfiles")
    parser.add_argument("--mcreweightvar", type=str, default="Pileup_nTrueInt", help=(
    ...
    if args.listmcprofiles:
        logger.info("The known PU profiles are: {}".format(", ".join(repr(k) for k in mcPUProfiles)))
        return
    elif args.mcfiles:
        if args.mcprofile:
            logger.warning("MC PU profile and MC files are passed - extracting from the files")
        logger.info("Extracting the MC profile from {} in the {} tree of: {}".format(
        ...
        raise RuntimeError(
            "Either one of --listmcprofiles or --mcprofile, "
            "or a list of MC files to extract a MC profile from, must be passed")
    ...
    if args.up and args.down:
        ...
        #     raise RuntimeError(
        #         "Up-variation and nominal binning is different: "
        #         "{0} vs {1}".format(upBins, nomBins)))
