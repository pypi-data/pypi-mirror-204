...
class DataSourceIwFactoryData(AbstractDataSource):
    ...
    def total(self):
        ...
        response = iuap_request.post_json(
            self.get_row_address, json={'tableid': self.table_id, 'tenantId': config.TENANT_ID}, auth_type=iuap_request.AuthType.YHT)
    ...
    def table_meta(self):
        response = iuap_request.post_json(
            self.get_table_meta, json={'logicIds': [self.table_id], 'tenantId': config.TENANT_ID}, auth_type=iuap_request.AuthType.YHT)
        if response.status != 200:
