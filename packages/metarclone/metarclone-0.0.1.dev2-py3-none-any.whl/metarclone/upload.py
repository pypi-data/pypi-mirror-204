...
def upload_walk(path: bytes, remote_path: str, st: os.stat_result, metadata: Optional[dict], conf: UploadConfig,
    ...
    if metadata and not conf.dest_as_empty:
        for filename, remote_file in metadata['files'].items():
            filename: str
            file_list: List[bytes] = [decode_child(i, conf.base32_encode) for i in remote_file['list']]
            keep = False
        ...
        for i in metadata['children']:
            decoded = decode_child(i, conf.base32_encode)
            if decoded not in child_list or not stat.S_ISDIR(stat_map[decoded].st_mode):
                remote_del(i, True)
    ...
    for child in child_list:
        ...
        encoded = encode_child(child, conf.base32_encode)
        if stat.S_ISDIR(child_st.st_mode):
            meta_child = metadata and metadata['children'].get(encoded)
            child_res = upload_walk(os.path.join(path, child), posixpath.join(remote_path, encoded),
            ...
            if child_res is not None:
                ...
                if child_res.force_retain:
                    ...
                    res.metadata['children'][encode_child(child, conf.base32_encode)] = child_res.metadata
    ...
    for i, child in enumerate(group_list):
        ...
        if group_size > conf.merge_threshold or i == len(group_list) - 1:
            ...
            while True:
                ...
                file_idx += 1
            # Update metadata
            meta_item = {'list': [encode_child(f, conf.base32_encode) for f in current_group]}
            if conf.use_file_checksum:
...
def upload_meta(path: bytes, remote_path: str, metadata: Optional[dict],
    ...
    # TODO: Check metadata config integrity
    st = os.stat(path)
    state = UploadState()
    ...
    res.metadata = {'version': conf.metadata_version,
                    ...
                    'base32_encode': conf.base32_encode,
                    'hard_links': [{'group': [encode_child(os.path.relpath(j, path), conf.base32_encode) for j in i]}
