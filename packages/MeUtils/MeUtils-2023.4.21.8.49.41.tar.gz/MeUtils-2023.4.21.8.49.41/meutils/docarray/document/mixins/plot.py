...
from meutils.docarray.document.mixins.mesh import MeshEnum, PointCloudEnum
        ...
        for f in self.non_empty_fields:
            if f.startswith('_'):
                ...
                if v.squeeze().ndim == 1 and len(v) < 1000:
                    from meutils.docarray.document.mixins.rich_embedding import (
                        ColorBoxEmbedding,
                    )
...
def _convert_display_uri(uri, mime_type):
    import urllib
    from meutils.docarray.document.mixins.helper import _to_datauri, _uri_to_blob
