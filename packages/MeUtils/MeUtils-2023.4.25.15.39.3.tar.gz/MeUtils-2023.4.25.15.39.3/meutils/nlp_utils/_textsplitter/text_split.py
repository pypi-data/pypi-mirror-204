...
def text_split(text, window_size=512, overlap_rate=0.25):  # 简单粗暴
    return str(text.strip()).lower() | xgroup(window_size, overlap_rate)


def split(sentences, max_length=64):  # 无overlap todo添加无overlap   往前重叠几个句子
    lens = list(map(len, sentences))
    sentences_ = sentences.copy()
    ls = []
    for i, length in enumerate(lens):
        idx = (np.cumsum(lens[i:]) >= max_length).argmax()

        if idx:
            ls.append(sentences_[:idx])
        else:
            ls.append(sentences_)
            break

        sentences_ = sentences_[idx:]  # 更新
        if not sentences_:
            break
...
#     num += len(s)
#     if num > max_len:  # 最大长度超过
#         _ = res[s_index: i]  # 截取
#         ss.append(_)
#
#     elif num == max_len:
#         _ = res[s_index: i + 1]  # 截取
#         ss.append(_)
#
#     _i = 0
#     for _i in range(i, 1, -1):  # 往前重复
#         if sum(map(len, res[s_index - _i: s_index])) < 45:
#             break
#
#     # 更新
#     num = len(s)
#     s_index = min(s_index - _i, 0)
