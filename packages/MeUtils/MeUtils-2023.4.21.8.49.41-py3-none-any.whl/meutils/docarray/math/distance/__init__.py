...
from meutils.docarray.math.ndarray import get_array_type

if TYPE_CHECKING:  # pragma: no cover
    from meutils.docarray.typing import ArrayType
    import numpy as np
...
def cdist(
    ...
    if metric == 'cosine':
        if framework == 'scipy' and is_sparse:
            from meutils.docarray.math.distance.numpy import sparse_cosine

            dists = sparse_cosine(x_mat, y_mat)
        elif framework == 'numpy':
            from meutils.docarray.math.distance.numpy import cosine

            dists = cosine(x_mat, y_mat)
        elif framework == 'tensorflow':
            from meutils.docarray.math.distance.tensorflow import cosine

            dists = cosine(x_mat, y_mat, device=device)
        elif framework == 'torch':
            from meutils.docarray.math.distance.torch import cosine

            dists = cosine(x_mat, y_mat, device=device)
        elif framework == 'paddle':
            from meutils.docarray.math.distance.paddle import cosine

            dists = cosine(x_mat, y_mat, device=device)

    elif metric == 'sqeuclidean':
        if framework == 'scipy' and is_sparse:
            from meutils.docarray.math.distance.numpy import sparse_sqeuclidean

            dists = sparse_sqeuclidean(x_mat, y_mat)
        elif framework == 'numpy':
            from meutils.docarray.math.distance.numpy import sqeuclidean

            dists = sqeuclidean(x_mat, y_mat)
        elif framework == 'tensorflow':
            from meutils.docarray.math.distance.tensorflow import sqeuclidean

            dists = sqeuclidean(x_mat, y_mat, device=device)
        elif framework == 'torch':
            from meutils.docarray.math.distance.torch import sqeuclidean

            dists = sqeuclidean(x_mat, y_mat, device=device)
        elif framework == 'paddle':
            from meutils.docarray.math.distance.paddle import sqeuclidean

            dists = sqeuclidean(x_mat, y_mat, device=device)

    elif metric == 'euclidean':
        if framework == 'scipy' and is_sparse:
            from meutils.docarray.math.distance.numpy import sparse_euclidean

            dists = sparse_euclidean(x_mat, y_mat)
        elif framework == 'numpy':
            from meutils.docarray.math.distance.numpy import euclidean

            dists = euclidean(x_mat, y_mat)
        elif framework == 'tensorflow':
            from meutils.docarray.math.distance.tensorflow import euclidean

            dists = euclidean(x_mat, y_mat, device=device)
        elif framework == 'torch':
            from meutils.docarray.math.distance.torch import euclidean

            dists = euclidean(x_mat, y_mat, device=device)
        elif framework == 'paddle':
            from meutils.docarray.math.distance.paddle import euclidean
