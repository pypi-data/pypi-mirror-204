from .CarDEC_CITE_optimization import grad_reconstruction as grad, MSEloss
from .CarDEC_CITE_dataloaders import simpleloader, aeloader
...
class SAE(Model):
    def __init__(self, dims, act = 'relu', actincenter = "tanh", 
                 ...
                 weights_dir = 'CarDEC_CITE Weights'):
    ...
    def call(self, x):
        ...
        ***Inputs***
            - x: `tf.Tensor`, an input tensor of shape (n_obs, p_HVG + p_HVP).
            
        ***Outputs***
            - output: `tf.Tensor`, A (n_obs, p_HVG + p_HVP) tensor of denoised HVG + HVP expression.
        """
    ...
    def embed(self, adata, batch_size = 64):
        """ This class method can be used to compute the low-dimension embedding for HVG + HVP features. 
        ...
        Returns:
        ------------------------------------------------------------------
        - embedding: `np.ndarray`, Array of shape (n_obs, n_vars) containing the cell HVG + HVP embeddings.
        """
    ...
    def makegenerators(self, adata, val_split, batch_size, splitseed):
        ...
        return aeloader(adata.layers["normalized input"][:, (adata.var['Variance Type'] == 'HVG') | (adata.var['Variance Type'] == 'HVP')], 
                        adata.layers["normalized input"][:, (adata.var['Variance Type'] == 'HVG') | (adata.var['Variance Type'] == 'HVP')], 
                        val_frac = val_split, batch_size = batch_size, splitseed = splitseed)
