...
def grad_MainModel(model, input_, target, target_p, total_loss, LVGP_target = None, aeloss_fun = None, clust_weight = 1.):
    """Function to do a backprop update to the main CarDEC model for a minibatch.
    
    
    Arguments:
    ------------------------------------------------------------------
    - model: `tensorflow.keras.Model`, The main CarDEC_CITE model.
    - input_: `list`, A list containing the input HVG+HVP and (optionally) LVG+LVP expression tensors of the minibatch for the CarDEC_CITE model.
    - target: `tf.Tensor`, Tensor containing the reconstruction target of the minibatch for the HVGs + HVPs.
    - target_p: `tf.Tensor`, Tensor containing cluster membership probability targets for the minibatch.
    - total_loss: `function`, Function to compute the loss for the main CarDEC_CITE model for a minibatch.
    - LVGP_target: `tf.Tensor` (Optional), Tensor containing the reconstruction target of the minibatch for the LVGs+LVPs.
    ...
    with tf.GradientTape() as tape:
        ...
        loss_value, aeloss = total_loss(target, denoised_output, target_p, cluster_output, 
                                LVGP_target, aeloss_fun, clust_weight)
...
def grad_reconstruction(model, input_, target, loss):
    ...
    ------------------------------------------------------------------
    - model: `tensorflow.keras.Model`, The main CarDEC_CITE model.
    - input_: `list`, A list containing the input HVG+HVP expression tensor of the minibatch for the CarDEC_CITE model.
    - target: `tf.Tensor`, Tensor containing the reconstruction target of the minibatch for the HVGs+HVPs.
...
def total_loss(target, denoised_output, p, cluster_output_q, LVGP_target = None, aeloss_fun = None, clust_weight = 1.):
    ...
    ------------------------------------------------------------------
    - target: `tf.Tensor`, Tensor containing the reconstruction target of the minibatch for the HVGs+HVPs.
    - denoised_output: `dict`, Dictionary containing the output tensors from the CarDEC_CITE main model's forward pass.
    ...
    if aeloss_fun is not None:
        ...
        aeloss_HVGP = aeloss_fun(target, denoised_output['HVGP_denoised'])
        if LVGP_target is not None:
            aeloss_LVGP = aeloss_fun(LVGP_target, denoised_output['LVGP_denoised'])
            aeloss = 0.5*(aeloss_LVGP + aeloss_HVGP)
        else:
            aeloss = 1. * aeloss_HVGP
    else:
        aeloss = 0.
