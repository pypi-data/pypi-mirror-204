from .CarDEC_CITE_utils import normalize_scanpy
from .CarDEC_CITE_MainModel import CarDEC_CITE_Model
from .CarDEC_CITE_count_decoder import count_model
...
class CarDEC_CITE_API:
    def __init__(self, adata, preprocess=True, weights_dir = "CarDEC_CITE Weights", batch_key = None, type_key = "type", 
                n_high_var_gene = 2000, n_high_var_protein = 150, LVG = True, LVP = True, 
                normalize_samples = True, log_normalize = True, normalize_features = True):
        """ Main CarDEC_CITE API the user can use to conduct batch correction and denoising experiments.
        ...
        - LVP: `bool`, If True, also model LVPs. Otherwise, only model HVPs.
        ...
        if n_high_var_gene is None:
            n_high_var_gene = None
            LVG = False

        if n_high_var_protein is None:
            n_high_var_protein = None
            LVP = False
        ...
        self.norm_args = (batch_key, type_key, n_high_var_gene, n_high_var_protein, 
                          LVG, LVP, normalize_samples, log_normalize, normalize_features)
    ...
    def build_model(self, load_fullmodel = True, dims = [128, 32], LVGP_dims = [128, 32], tol = 0.005, n_clusters = None, 
        ...
        """ Initializes the main CarDEC_CITE model.
        ...
        pG = sum(self.dataset.var["Variance Type"] == 'HVG')
        pP = sum(self.dataset.var["Variance Type"] == 'HVP')

        self.dims = [pG + pP] + dims
        
        # we only have two option, self.LVG = self.LVP = True, or self.LVG = self.LVP = False for current version of CarDEC_CITE
        if self.LVG and self.LVP:
            LVG_p = sum(self.dataset.var["Variance Type"] == 'LVG')
            LVP_p = sum(self.dataset.var["Variance Type"] == 'LVP')

            self.LVGP_dims = [LVG_p + LVP_p] + LVGP_dims
        else:
            self.LVGP_dims = None
        
        self.load_fullmodel = load_fullmodel
        self.weights_exist = os.path.isfile("./" + self.weights_dir + "/tuned_CarDEC_CITEweights.index")
        
        set_centroids = not (self.load_fullmodel and self.weights_exist)
        
        self.model = CarDEC_CITE_Model(self.dataset, self.dims, self.LVGP_dims, tol, n_clusters, random_seed, louvain_seed, 
    ...
    def make_inference(self, batch_size = 64, val_split = 0.1, lr = 1e-04, decay_factor = 1/3,
        ...
        """ This class method makes inference on the data (batch correction + denoising) with the main CarDEC_CITE model
        ...
        if not self.loaded:
            if self.load_fullmodel and self.weights_exist:
                ...
                print("CarDEC_CITE Model Weights not detected. Training full model.\n")
        ...
        if denoise_list is not None:
            ...
            if self.LVG and self.LVP:
                hvgp_ds = tf.data.Dataset.from_tensor_slices(self.dataset.obsm["embedding"][indices])
                lvgp_ds = tf.data.Dataset.from_tensor_slices(self.dataset.obsm["LVGP embedding"][indices])
            
                input_ds = tf.data.Dataset.zip((hvgp_ds, lvgp_ds))
                input_ds = input_ds.batch(batch_size)

                start = 0     
                for x in input_ds:
                    denoised_batch = {'HVGP_denoised': self.model.decoder(x[0]), 'LVGP_denoised': self.model.decoderLVG(x[1])}
                    q_batch = self.model.clustering_layer(x[0])
                    end = start + q_batch.shape[0]

                    denoised.iloc[start:end, np.where((self.dataset.var['Variance Type'] == 'HVG') | (
                        self.dataset.var['Variance Type'] == 'HVP'))[0]] = denoised_batch['HVGP_denoised'].numpy()
                    denoised.iloc[start:end, np.where((self.dataset.var['Variance Type'] == 'LVG') | (
                        self.dataset.var['Variance Type'] == 'LVP'))[0]] = denoised_batch['LVGP_denoised'].numpy()
                ...
                for x in input_ds:
                    denoised_batch = {'HVGP_denoised': self.model.decoder(x)}
                    q_batch = self.model.clustering_layer(x)
                    end = start + q_batch.shape[0]

                    denoised.iloc[start:end] = denoised_batch['HVGP_denoised'].numpy()
