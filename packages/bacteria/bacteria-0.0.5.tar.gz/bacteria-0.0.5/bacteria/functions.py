...
def growth_rate_old(df, good_cells, fps = 3):
    """
    Calculate the growh rate for each cell in the dataset
    using a window of 3 data points. 
    ...
    time_dict = {}
    for cell in good_cells:
        y = np.log(df[df['Cell ID']==cell]['Long axis (L)'].values)
    ...
    gr = np.zeros((len(gr_inst),len(time)))

    for idx,cell in enumerate(good_cells):
        for _,data in enumerate(zip(time_dict[cell],gr_inst[cell])):
            gr[idx,np.where(time==data[0])[0][0]] = data[1]
    
    return gr,time

def plot_growth_rate_old(df_3d,good_cells,fps=3, ax = None):
    """
    Plot the growth rate for the entire experiment.
    Check also growth_rate_old().
    ...
    df_3d : DataFrame
        DataFrame of 3D data
    good_cells : nd.array
        Vector with the ID of the good cells (output of combined_filters(),
        concatenate_clist() or df_data3d(), different cells for each func.)
    fps: int (optional)
        Frames per second used in the experiment, usual 3
    ...
    None
    """
    gr,time = growth_rate_old(df_3d,good_cells,fps=fps)

    if ax is None:
        plt.plot(time,np.mean(gr,0))
        plt.title('Instantaneous Growth Rate', fontsize = 17)
        plt.xlabel('Time (min)', fontsize = 15)
        plt.ylabel('Instantaneous Growth (a.u.)', fontsize = 15)
        plt.show()
    else:
        out = ax.plot(time,np.mean(gr,0)),\
              ax.set_xlabel('Time (min)', fontsize = 15),\
              ax.set_ylabel('Instantaneous Growth (a.u.)', fontsize = 15),\
              ax.set_title('Instantaneous Growth Rate', fontsize = 17)
        
        return out
    
def growth_rate_single_cell_old(df, id, ax = None):
    """
    Plot the growth rate for a single cell (coef). 
    ...
    """
    model = LinearRegression()
    y = np.log(df[df['Cell ID']==id]['Long axis (L)'].values)
    x = df[df['Cell ID']==id]['Time (fps)'].values
    model.fit(x.reshape(len(x), 1), y.reshape(len(x)))

    if ax is None:
        plt.plot(x, model.predict(x.reshape(len(x), 1)), color='k', linewidth=1,label='Coef: ' + str(model.coef_[0])[:5])
        plt.plot(df[df['Cell ID']==id]['Time (fps)'].values,np.log(df[df['Cell ID']==id]['Long axis (L)'].values),label='Cell: '+str(int(id)))
        plt.xlabel('Time (min)',fontsize = 15)
        plt.ylabel('Cell size',fontsize = 15)
        plt.title('Growth Rate',fontsize = 17)
        plt.legend()
    else:
        out = ax.plot(x, model.predict(x.reshape(len(x), 1)), color='k', linewidth=1,label='Coef: ' + str(model.coef_[0])[:5]),\
              ax.plot(df[df['Cell ID']==id]['Time (fps)'].values,np.log(df[df['Cell ID']==id]['Long axis (L)'].values),label='Cell: '+str(int(id))),\
              ax.set_xlabel('Time (min)',fontsize = 15),\
              ax.set_ylabel('Cell size',fontsize = 15),\
              ax.set_title('Growth Rate',fontsize = 17)
...
def derivative(df_3d, column = 'Fluor1 sum', norm = True, log = False,minus = 1, plus = 2):
    """
    ...
    norm : bool (optional)
        If true will normalize the derivative using 
        the volume. Default 'True'
    log : bool (optional)
        If true will estimate the derivative of the
        log ('np.log()') of the window. Default 'False'
    minus : int (optional)
        value to change the window to the convolution
        to adapt the range for the loop 
        (e.g, default = i in range(1,len(x)-2))
    ...
    --------------
    vol_dict : dict
        volume of each cell at the slide window (cell number = key)
    dev_dict : dict
        fluor value of each cell at the slide window (cell number = key)
    time_dict : dict
        moment in time of each cell at the slide window (cell number = key)
    ...
    vol_dict,dev_dict,time_dict,bin_dict,ratio_dict = {},{},{},{},{}
    ...
    if len(cells) < 50:
        for idx in range(len(cells)):
            ...
            for i in range(minus,len(time_frames)-plus):
                    ...
                    if norm:
                        if column == 'Volume':
                            deriv.append(model_deriv.coef_[0]/df_dev[df_dev['Cell ID'] == cell].value.values[i])
                        else:
                            deriv.append(model_deriv.coef_[0]/df_dev[df_dev['Cell ID'] == cell]['Volume'].values[i])  
                    else:
        ...
        for idx in progressbar(range(len(cells)), "Computing " + str(len(cells)) + " cells: ", 50):
            cell = cells[count]
            count += 1
            time_frames = df_dev[(df_dev['Cell ID'] == cell) & (df_dev['Time (fps)'])]['Time (fps)'].values
            vol,deriv,time,ratio = [],[],[],[]
    ...
    for cell in vol_dict.keys():
        ...
        df_temp.append(pd.DataFrame(list(zip(ref_list,time_dict[cell],ratio_dict[cell],vol_dict[cell],dev_dict[cell],bin_dict[cell])),
                                    columns=['Cell ID','Time (fps)','F/V','Volume','Derivative','Cell Cycle']))
        
    df_derivative = pd.concat(df_temp, ignore_index=True)

    return vol_dict,dev_dict,time_dict,df_derivative,plot_params
...
def	derivative_binning(df_deriv, bins = 10 , plot_params = None, sort_by = 'Cell Cycle', print_bins = True, conf = 0.95, method = np.mean):
    """
    ...
    plot_params : dict
        Dict with the parameters used to estimate
        the derivative (column name, norm, log, 
        bin or just bin).
    """
    plot_params_bin = {}
    if plot_params is None:
        plot_params_bin['bin'] = sort_by
    for key,data in plot_params.items():
        plot_params_bin[key] = data
    plot_params_bin['bin'] = sort_by

    if sort_by == 'Time (fps)':
        df_bin = pd.melt(df_deriv, id_vars=['Cell ID','Time (fps)','Volume','Cell Cycle'], value_vars=['Derivative']).sort_values(by=[sort_by])
    ...
    return bins_mean,ci,plot_params_bin
...
def growth_rate(df_3d,column = 'Volume',norm = True,log = False,minus = 1,plus = 2,conf = .95, method = np.mean, plot_hist = False):
	"""
	Estimate the growth rate mean for all cells using
	the 'pd.melt()' method.
	...
	df_3d : DataFrame
		DataFrame of 3D data
	column : str (optional)
		DataFrame column to estimate the derivative,
		default is 'Volume'
	norm : bool (optional)
		If true will divide the derivative by the Volume,
		default is True
	log : bool (optional)
		If true will make the 'np.log()' of the data,
		default is False
		window and then estimate the derivative.
	minus : int (optional)
		value to change the window to the convolution
		to adapt the range for the loop 
		(e.g, default = i in range(1,len(x)-2))
    ...
    plot_params : dict
        Dict with the parameters used to estimate the derivative
        of the lineage (column name, norm, log). Output of 
        'derivative(), should be used for plot and in the
        'derivative_binning()'.
	df : DataFrame
        DataFrame with the derivative (grow rate) and the
        specific value for Time (fps), Volume and Cell Cycle
	"""
	_,_,_,df,plot_params = derivative(df_3d,column=column,norm =norm,log=log,minus=minus,plus=plus)

	len_data = []
	time_ref = natsorted(set(df['Time (fps)'].values))
	growth_rate_mean = np.zeros((len(time_ref),))
	ci = np.zeros((len(time_ref),2))

	for idx,ts in enumerate(time_ref):
		temp = df[df['Time (fps)']==ts]['Derivative'].values
		growth_rate_mean[idx] = np.mean(temp)
		len_data.append(len(df[df['Time (fps)']==ts]['Derivative'].values))
		if len(temp) > 1:
			ci[idx,:] = ci_bootstrap(temp,conf = conf, method = method, plot = False)
		else:
			ci[idx,:] = [-temp[0],+temp[0]]
	...
	return time_ref,growth_rate_mean,ci,plot_params,df

def plot_derivative_mean(time,mean,ci,plot_params,color = 'Orange', ax = None):
    """
    ...
    if ax is None:
        ...
        if plot_params['column'] == 'Volume':
            plt.title('{} Derivative'.format(plot_params['column']), fontsize = 17)
        else:
            plt.title('Growth Rate'.format(plot_params['column']), fontsize = 17)
        plt.xlabel('Time (min)', fontsize = 15)
        ...
        if plot_params['column'] == 'Volume':
            ...
            out.append(ax.set_title('{} Derivative'.format(plot_params['column']), fontsize = 17))
        out.append(ax.set_xlabel('Time (min)', fontsize = 15))
        if plot_params['norm'] and plot_params['log']:
...
def plot_bins(arr,ci,plot_params = None,cmap = 'rainbow',line = 'k',ax =None):
	"""
    ...
    plot_params : dict (optional)
        Parameters to plot the corect axis, output of
        'derivative_binning()'.
	cmap : str (optional)
	...
	if ax is None:
		...
		if plot_params is not None:
			if plot_params['bin'] == 'Time (fps)':
				plt.xlabel('Time (min)', fontsize = 15)
			elif plot_params['bin'] == 'Volume':
				plt.xlabel(r'Volume [$\mu m^3$]', fontsize = 15)
			else:
				plt.xlabel(plot_params['bin'], fontsize = 15)
		else:
			plt.xlabel('Bins values', fontsize = 15)
		if plot_params is not None:
			if plot_params['norm'] and plot_params['log']:
				plt.ylabel(r'$\frac{\frac{\partial (log(%s))}{\partial t}}{Volume}$'%(plot_params['column']), fontsize = 15)
			elif plot_params['norm']:
				plt.ylabel(r'$\frac{\frac{\partial %s}{\partial t}}{Volume}$'%(plot_params['column']), fontsize = 15)
			elif plot_params['log']:
				plt.ylabel(r'$\frac{\partial (log(%s))}{\partial t}$'%(plot_params['column']), fontsize = 15)
			else:
				plt.ylabel(r'$\frac{\partial %s}{\partial t}$'%(plot_params['column']), fontsize = 15)
		else:
		...
		if plot_params is not None:
			if plot_params['bin'] == 'Time (fps)':
				...
				out.append(ax.set_xlabel(plot_params['bin'], fontsize = 15))
		else:
			out.append(ax.set_xlabel('Bins values', fontsize = 15))
		if plot_params is not None:
			if plot_params['norm'] and plot_params['log']:
				out.append(ax.set_ylabel(r'$\frac{\frac{\partial (log(%s))}{\partial t}}{Volume}$'%(plot_params['column']), fontsize = 15))
			elif plot_params['norm']:
				out.append(ax.set_ylabel(r'$\frac{\frac{\partial %s}{\partial t}}{Volume}$'%(plot_params['column']), fontsize = 15))
			elif plot_params['log']:
				out.append(ax.set_ylabel(r'$\frac{\partial (log(%s))}{\partial t}$'%(plot_params['column']), fontsize = 15))
			else:
				out.append(ax.set_ylabel(r'$\frac{\partial %s}{\partial t}$'%(plot_params['column']), fontsize = 15))
		else:
			out.append(ax.set_ylabel('Data', fontsize = 15))
...
def plot_lineage_derivative_old(df_3d,reverse_lineage,column = 'Fluor1 sum',window = 19, order = 3, norm = True, ax = None):
	"""
	...
	order : int (optional)
		...
		savgol_filter from scipy. Default 3 based
		on previous tests
	norm : bool
		If true will normalize the derivative values
		using the volume data.
	ax : nd.array (optional)
		Pass the ax for subplot.
	...
	time_concat = []
	data_concat = []
	vol,dev,time,_,_ = derivative(df_3d[df_3d['Cell ID'].isin(reverse_lineage[:-1])],column=column)

	for i in dev:
		time_concat.extend(time[i])
		if norm:
			data_concat.extend(dev[i]/vol[i])
		else:
			data_concat.extend(dev[i])

	yhat = savgol_filter(data_concat, window, order) # window size 51, polynomial order 3

	if ax is None:
		for i in dev:
			if norm:
				plt.plot(time[i],dev[i]/vol[i],'.',label = 'Cell {}'.format(i))
			else:
				plt.plot(time[i],dev[i],'.',label = 'Cell {}'.format(i))
		plt.plot(time_concat,yhat,'k')
		plt.title('Lineage {}-{} Derivative'.format(reverse_lineage[0],reverse_lineage[-1]),fontsize = 17)
		plt.xlabel('Time (fps)',fontsize = 15)
		if norm:
			plt.ylabel('Derivative/V',fontsize = 15)
		else:
		...
		for i in dev:
			if norm:
				out.append(ax.plot(time[i],dev[i]/vol[i],'.',label = 'Cell {}'.format(i)))
			else:
				out.append(ax.plot(time[i],dev[i],'.',label = 'Cell {}'.format(i)))
		out.append(ax.plot(time_concat,yhat,'k'))
		out.append(ax.set_title('Lineage {}-{} Derivative'.format(reverse_lineage[0],reverse_lineage[-1]),fontsize = 17))
		out.append(ax.set_xlabel('Time (fps)',fontsize = 15))
		if norm:
			out.append(ax.set_ylabel('Derivative/V',fontsize = 15))
		else:
			out.append(ax.set_ylabel('Derivative',fontsize = 15))
		out.append(ax.legend())
...
def lineage_derivative(df_3d, reverse_lineage, column = 'Fluor1 sum',norm = True,log = False,std = 2,window = None, order = 5, perc = .7, mode = 'nearest'):
    """
    Plot a single lineages derivative. It's possible to chose
    different columns to inspect. The function is already removing
    the last value of the reverse_lineage, because the last
    ...
    plot_params : dict
        Dict with the parameters used to estimate the derivative
        of the lineage (column name, norm, log). Output of 
        'lineage_derivative()'. Also, this dictionary has the 
        info of the order used to create the summed volume for
        the X axis (key 'volume order').
    """	
    ...
    _,_,_,df_deriv,_ = derivative(df_3d[df_3d['Cell ID'].isin(keys)],column=column,norm=norm,log=log)
    ...
    plot_params['volume order'] = x_vol_order
    vol_dict,dev_dict,time_dict,ratio_dict = {},{},{},{}
    vol_concat,dev_concat,time_concat,ratio_concat,df_temp = [],[],[],[],[]

    for cell in keys:
        temp_vol = df_deriv[df_deriv['Cell ID']==cell]['Derivative'].values
    ...
    return df_lineage,peaks,plot_params
	...
	if derivative_plot:
		...
		for idx,lineage in enumerate(temp_lineage[::2]):
			if lineage_plot_size != 1:
				df_lineage,extremes,plot_params = lineage_derivative(df_3d,temp_lineage[::2][idx],column='Volume', norm=False,order=4)
		...
		for idx,lineage in enumerate(temp_lineage[::2]):
			if lineage_plot_size != 1:
				...
				df_lineage,extremes,plot_params = lineage_derivative(df_3d,temp_lineage[::2][idx],column='Fluor1 sum', norm=False,order=4)
				y_axis = pre.MinMaxScaler((0,50)).fit_transform(df_lineage['Smoothed Derivative'].values.reshape(-1, 1))
