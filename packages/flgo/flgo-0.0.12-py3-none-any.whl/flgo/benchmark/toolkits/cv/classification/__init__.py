...
def gen_benchmark(benchmark_name:str, rawdata_path:str, builtin_class, generator_config:dict, model:list=[], target_path:str='.'):
    """ Automatically create a benchmark on the target path.

    Args:
        rawdata_path
    """
    if os.path.exists(os.path.join(target_path, benchmark_name)):
        raise FileExistsError("The benchmark named {} in {} already exists.".format(benchmark_name, target_path))
    # create core.py

    # if model is not [], create model
    # create __init__.py
