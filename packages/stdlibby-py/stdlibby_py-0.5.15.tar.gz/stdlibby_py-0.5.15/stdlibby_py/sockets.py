import readline
import asyncio, os, sys, random, string
...
def gen_socket(dir: str="/tmp/sockets"):
    os.makedirs(dir, exist_ok=True)
    file = f"{random.choices(list(string.ascii_lowercase), k=4)}.socket"
    path = os.path.join(dir, file)
    if os.path.exists(path):
        return gen_socket(dir=dir)
    else:
        return path

async def run_unix_server(handler: Callable[[asyncio.StreamReader, asyncio.StreamWriter], Coroutine[Any, Any, Any]]=echo_handler, socket_path: str=gen_socket()):
    print(f"server starting at {socket_path}")
