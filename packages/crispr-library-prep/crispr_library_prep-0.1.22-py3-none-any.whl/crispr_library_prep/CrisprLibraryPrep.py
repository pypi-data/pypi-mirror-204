...
@typechecked
def get_target_coverage(gDNA_amount: float, moi: float, sample_name:str, perfection_rate: float, guide_library_size:int, genome_conc_ng: float = _GENOME_CONC_NG, target_coverage_input=None, target_coverage_percentage: float=0.70, read_intervals_count: int= 50, approximation=False, max_reads_multiplier: Union[int, float]= 3, cores=1):
    ...
    max_reads = PCR1_input_coverage * guide_library_size * max_reads_multiplier
...
@typechecked
def get_target_coverage_per_sample(ideal_gDNA_amount: Union[None, float], max_gDNA_amount: float, moi: float, sample_name:str, perfection_rate: float, guide_library_size:int, genome_conc_ng: float = _GENOME_CONC_NG, target_coverage_input=None, target_coverage_percentage: float=0.70, max_target_coverage_input: int = 600, read_intervals_count: int= 50, gDNA_intervals_count: int = 5, approximation=False, limit_max_gDNA_amount: bool = True, max_reads_multiplier: Union[float, int] = 3, cores=1):
    print(f"Processing sample: {sample_name}")

    # Truncate max_gDNA_amount if too high
    if limit_max_gDNA_amount:
        max_input_coverage = ((max_target_coverage_input*1.5) * (1/(target_coverage_percentage))) # Get the max input coverage based on the max desired coverage and the target_coverqage_percentage (overshoot a bit by multiplying max coverage by 1.5)
    ...
    get_target_coverage_p = partial(get_target_coverage, moi=moi, sample_name=sample_name, perfection_rate=perfection_rate, guide_library_size=guide_library_size, genome_conc_ng=genome_conc_ng, target_coverage_input=target_coverage_input, target_coverage_percentage=target_coverage_percentage, read_intervals_count=read_intervals_count, approximation=approximation, max_reads_multiplier=max_reads_multiplier, cores=1)
...
def process_sample_sheet(sample_sheet: pd.DataFrame, target_coverage_percentage: float, max_target_coverage_input: float, suggested_target_coverage_amounts: List[Union[float, int]], read_intervals_count: int =50, approximation:bool=False, limit_max_gDNA_amount: bool=True, max_reads_multiplier: Union[float, int]=3, cores=1):
    def process_row(row):
        return get_target_coverage_per_sample(
            ...
            limit_max_gDNA_amount=limit_max_gDNA_amount,
            max_reads_multiplier=max_reads_multiplier,
            cores=cores)
