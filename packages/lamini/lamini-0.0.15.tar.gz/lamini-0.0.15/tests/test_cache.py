...
class TestCache(MultiModelTest):

    @run_models(models=["hf-internal-testing/tiny-random-gpt2"], run_with_default=True)
    def test_cache(self):
        ...
        class MetaData(Type):
            type: str = Context(
                "Type of Page i.e Customer URL/ Competitor URL")
            brand: str = Context(
        ...
        def create_keyword_data(keyword_data):
            keyword_data = (
                keyword_data["ranked_keywords"] if "ranked_keywords" in keyword_data else {
                }
            )
            return KeywordData(
                __root__={
        ...
        def create_meta_elements(meta_elements):
            return MetaElements(
                ...
                meta_description=create_data(
                    meta_elements["meta_description"]),
            )
        ...
        def create_urlnme_dict(html_elements):
            return {url: create_metadata(metadata)
                    for url, metadata in html_elements.items() if metadata['type'] == 'customer'}
        ...
        def get_datum(customer_info):
            ...
            if "html_elements" in customer_info:
                html_elements = create_urlnme_dict(
                    customer_info["html_elements"])
        ...
        full_header_metadata = llm(
            input=customer_info, output_type=Numbers_Recommendations_Meta, random=True
        )
        ...
        print(
            f"Uncached without logprobs took {uncached_time}: {full_header_metadata}")

        start = time()

        full_header_metadata = llm(
            input=customer_info, output_type=Numbers_Recommendations_Meta, random=True, score='score'
        )
        ...
        print(
            f"Uncached with logprobs took {uncached_time}: {full_header_metadata}")

        start = time()

        full_header_metadata = llm(
            input=customer_info, output_type=Numbers_Recommendations_Meta, random=False, score='score'
        )
        ...
        print(
            f"Cached with logprobs took {cached_time}: {full_header_metadata}")

        assert cached_time <= uncached_time, 'Cached time is too long'
