...
class CompilerLangVy(CompilerLang, metaclass=Singleton):
                ...
                'members': [
                    {
                        ...
                        'offset': 0,
                        'slot': '0',
                        'type': 'uint256_bounded_1_' + str(self.array_type.max_num_elements)
                    },
                    {
                        'label': 'data',
                        'offset': 0,
    ...
    class VyperTypeBoundedInteger(VyperTypePrimitive):
        def __init__(self, name: str, size: int, lower_bound: int, upper_bound: int):
    ...
    def collect_storage_layout_info(file_abs_path: str,
        ...
        def generate_bounded_integer_types(ts: List[CompilerLangVy.VyperType]) -> \
                Dict[str, CompilerLangVy.VyperTypeBoundedInteger]:
            return dict(
                [('uint256_bounded_1_' + str(t.array_type.max_num_elements),
                  CompilerLangVy.VyperTypeBoundedInteger('uint256_bounded_1_' + str(t.array_type.max_num_elements),
                                                         32, 1, t.array_type.max_num_elements))
                 for t in ts if isinstance(t, CompilerLangVy.VyperTypeDynArray)]
            )

        def generate_storage_fields(storage_entries: Dict[str, Any]) -> List[Dict[str, Any]]:
            return [{
                'label': label,
                'slot': str(storage_entries[label]['slot']),
                'offset': 0,
                'type': storage_entries[label]['type']
            } for label in storage_entries.keys()]
        ...
        all_used_types = list(itertools.chain.from_iterable([e.get_used_types() for e in resolved_types])) + \
            ...
            list(generate_bounded_integer_types(resolved_types).values())
        types_field = {i.get_canonical_vyper_name(): i.generate_types_field() for i in all_used_types}
        storage_field = generate_storage_fields(storage_layout_dict['storage_layout'])
