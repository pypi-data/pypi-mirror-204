...
class ProjectInstanceWindow(models.Model):
    _name = 'project.instance.window'
    _description = 'Available Deployment Window'

    instance_id = fields.Many2one(
        'project.instance', string='Instance', ondelete='cascade')
    start_time = fields.Datetime('Start Time')
    end_time = fields.Datetime('End Time')

    def open_calendar_event_wizard(self):
        self.ensure_one()
        wizard = self.env['project.instance.window.wizard'].create({
        ...
        return {
            ...
            'res_model': 'project.instance.window.wizard',
            'res_id': wizard.id,
            'view_mode': 'form',
            'target': 'new',
