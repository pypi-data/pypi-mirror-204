...
from .... import fromjson, editfolder, editresponder, editpage, utils
...
def retrieve_add_page(skicall):
    ...
    try:
        foldernumber = utils.get_itemnumber(project, call_data['edited_folder'])
        if foldernumber is None:
            raise FailPage(message="Parent folder not recognised")
        folder_info = utils.folder_info(project, foldernumber)
        folder_url = utils.page_path(project, foldernumber)
    ...
    if 'page_ident_number' in call_data:
        ...
        pd['it3','page_ident_number'] = str(utils.next_ident_number(project))
...
def _common_page_items(skicall):
    ...
    if foldernumber is None:
        raise FailPage(message="Parent folder not recognised")
    # utils.folder_info raises a ServerError if this is not a folder
    try:
        folder_info = utils.folder_info(project, foldernumber)
    except ServerError as e:
        raise FailPage(message=e.message)
...
def retrieve_new_svg(skicall):
    ...
    parent_url = utils.page_path(project, foldernumber)
...
def retrieve_new_responder(skicall):
    ...
    # get a list of lists, each inner being [responder name, module name] from utils.editresponder
