...
from .... import editfolder, fromjson, utils
...
def _get_folder_info(project, folder):
    ...
    try:
        foldernumber = utils.get_itemnumber(project, folder)
...
def retrieve_add_folder(skicall):
    ...
    if 'folder_ident_number' in call_data:
        ...
        pd['it2','folder_ident_number'] = str(utils.next_ident_number(project))
...
"""
    ...
    if 'folderpath' in call_data and call_data['folderpath']:
        ...
        fullpath = os.path.join(utils.get_projectfiles_dir(project), folderpath)
...
"""
    try:
        ...
        ident_number_list = utils.ident_numbers(project)
        for root, dirs, files in os.walk(fullpath):
            #fpath = root[len(utils.get_projectdir(project))+1:]
            fpath = root[len(utils.get_projectfiles_dir(project))+1:]
