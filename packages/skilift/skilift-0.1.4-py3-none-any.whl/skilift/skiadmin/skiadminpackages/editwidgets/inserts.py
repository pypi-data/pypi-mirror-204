...
from .... import editpage, editsection, utils
...
def insert_in_widget(skicall):
    ...
    part_tuple = utils.part_info(editedprojname, pagenumber, section_name, location)
...
def insert_text(skicall):
    ...
    if "page_number" in call_data:
        pagenumber = call_data["page_number"]
        page_info = utils.item_info(editedprojname, pagenumber)
    ...
    if pagenumber:
        call_data['pchange'], new_location = utils.insert_item_in_page(editedprojname, pagenumber, call_data['pchange'], location, new_text)
