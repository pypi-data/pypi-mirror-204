...
from skipole import WSGIApplication, FailPage, GoTo, ValidateError, ServerError, ServeFile, use_submit_list, PageData, SectionData, set_debug

from ..fromjson import get_defaults_from_file
...
def start_call(called_ident, skicall):
    ...
    projinfo = utils.project_info(editedprojname)
...
def make_skiadmin(editedprojname, examples=None):
...
def set_navigation(identnum, call_data, pd, sd_adminhead):
    ...
    if item_number:
        ...
        parents = utils.parent_list(editedprojname, item_number)
        for item in parents:
    ...
    if item_number:
        item_info = utils.item_info(editedprojname, item_number)
        if item_info:
            if (item_info.item_type == 'TemplatePage') or (item_info.item_type == 'SVG'):
                ...
                if ('widget_name' in call_data) and ((item_info.item_type == 'TemplatePage') or (item_info.item_type == 'SVG')):
                    widget_info = utils.widget_info(editedprojname, item_number, None, call_data['widget_name'])
        ...
        if 'widget_name' in call_data:
            widget_info = utils.widget_info(editedprojname, None, call_data['section_name'], call_data['widget_name'])
...
def display_parent(widget_info, sd):
    ...
    parent_info = utils.widget_info(widget_info.project, widget_info.pagenumber, widget_info.section_name, parent_name)
