...
class GPT35TurboEngine(Engine):
    ...
    def forward(self, prompts: List[str], *args, **kwargs) -> List[str]:
        ...
        while not success and retry < max_retry:
            ...
            try:
                ...
                errors.append(e)
                self.logger.warn(f"GPT-3.5-Turbo service is unavailable or caused an error. Retry triggered: {e}")
                sleep(self.api_cooldown_delay) # API cooldown
                # remedy for GPT-3.5-Turbo API limitation
        ...
        if not success:
            msg = f"Failed to query GPT-3.5-Turbo after {max_retry} retries. Errors: {errors}"
            # interpret error
            from symai.symbol import Symbol
    ...
    def prepare(self, args, kwargs, wrp_params):
        ...
        if wrp_params['system'] is not None: # TODO: check if this works
            system += str(wrp_params['system'])
        # build operation
