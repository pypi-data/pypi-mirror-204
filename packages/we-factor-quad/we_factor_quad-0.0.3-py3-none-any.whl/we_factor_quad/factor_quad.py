...
class FactorQuad(object):
    ...
    def __init__(self,
        ...
        if len(missing_factors) > 0:
            beta_lst = []
            for date in list(set(self.beta_ts['date'])):
                beta_t = self.beta_ts[self.beta_ts['date'] == date]
        ...
        self.beta_ts = self.beta_ts.drop_duplicates([self._time_col_name, self._code_col_name], ignore_index=True)
        ...
        # 基本维度信息
        self.assets_n = self.beta_ts.shape[1]
        self.factors_m = self.beta_ts.shape[0]
