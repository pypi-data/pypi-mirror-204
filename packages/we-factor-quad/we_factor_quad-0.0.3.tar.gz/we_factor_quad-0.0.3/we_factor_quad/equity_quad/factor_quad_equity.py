...
class FactorQuadEQ(FactorQuad):
    ...
    def get_cross_sec_sigma_with_country(self, data: pd.DataFrame):
        ...
        df_factors = df.drop(['date', 'source'], axis='columns')
    ...
    def add_country_factor(self):
        ...
        sigma_cols = ['date', 'source'] + industry_list + style_list
        sigma = self.rearrange_cols(sigma, sigma_cols)

        dates = list(sigma['date'].unique())

        switch = True
        sigma_with_country = None
        for date in dates:
            sigma_slice = sigma[sigma['date'] == date].copy().reset_index(drop=True)
        ...
        beta_cols = ['date', 'code'] + industry_list + style_list
...
def test_FactorQuadEQ():
    def load_data():
        ...
        local_path = r'C:\Users\Administrator\seadrive_root\yangyn\群组资料库'
        myquad = FactorQuadEQ.create_factor_quad(factor_system=StocksOutputReport.we_factor_case_name,
    ...
    def load_data_to_csv():
        myquad = FactorQuadEQ.create_factor_quad(factor_system='HF25_SRAM',
