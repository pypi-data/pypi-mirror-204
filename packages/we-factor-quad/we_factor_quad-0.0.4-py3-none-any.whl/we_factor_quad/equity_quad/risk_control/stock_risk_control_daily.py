    ...
    def get_stock_report(self) -> Dict:
        ...
        for stock in self.stocks:
            stock_risk_dic = {'0Close': self.px_close[stock],
                              '3Forward 3M Vol': rolling_vol[stock]}
            for i in range(len(self.decomp_names)):
                stock_risk_dic[f'1{self.decomp_names[i]} Vol'] = np.sqrt(self.total_var_list[i])[stock]
                stock_risk_dic[f'2{self.decomp_names[i]} iVol'] = np.sqrt(self.idio_var_list[i])[stock]
                stock_w, risk_managed_performance = self._get_stock_risk_control_result(stock_code=stock,
                                                                                        target_vol=0.1)
                stock_risk_dic[f'4{self.decomp_names[i]} ivol_pct'] = np.sqrt(self.idio_var_list[i])[stock] / \
                                                                      np.sqrt(self.total_var_list[i])[stock]
                stock_risk_dic[f'5{self.decomp_names[i]} weight'] = stock_w[i]
                stock_risk_dic[f'6{self.decomp_names[i]} performance'] = risk_managed_performance[i]
