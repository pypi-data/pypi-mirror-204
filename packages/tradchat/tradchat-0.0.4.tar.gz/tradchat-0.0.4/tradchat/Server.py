...
def signup():
    ...
    with suppress(KeyError):
        ...
        return redirect('/mainpage/')
    if request.method == 'POST':
        if request.form['username'].lower() in accounts: return 'Please don`t try anything smart. We want no hackers on tradchat'
        session['username'] = request.form['username'].lower()
...
def recv(msg):
    ...
    if message[0] == "Checking to see if my password is correct":
        if (message[1].lower() in accounts) and (message[2].lower() == accounts[message[1].lower()][0].lower()):
    ...
    if message[0] == "Making sure that your username is not already claimed":
        if (message[1].lower() in accounts):
