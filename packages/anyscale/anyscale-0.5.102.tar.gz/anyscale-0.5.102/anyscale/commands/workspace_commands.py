...
def run(command: str, web_terminal: bool, as_job: bool, no_push: bool,) -> None:
    ...
    if web_terminal:
        ...
        workspace_controller.run_cmd(f"cd ~/{dir_name} && {command}")
...
def ssh() -> None:
    ...
    workspace_controller = WorkspaceController()
    dir_name = workspace_controller.get_workspace_dir_name()
    workspace_controller.run_cmd(f"cd ~/{dir_name} && /bin/bash")
...
def _do_pull(pull_git_state):
    ...
    if pull_git_state:
        workspace_controller.run_cmd(f"cd ~/{dir_name} && git repack -a -d")
