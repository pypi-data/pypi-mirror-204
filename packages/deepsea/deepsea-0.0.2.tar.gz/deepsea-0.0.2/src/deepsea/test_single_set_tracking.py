import os
import deepsea.tracker_transforms as tracker_transforms
import numpy as np
import argparse
import random
import cv2
...
import deepsea.trained_models as trained_models
...
def apply_cell_tracking(img_list,tr_image_size = [128,128],image_means = [0.5],image_stds= [0.5]):
    """ function to cell tracking process to a single set of time lapse microscopy image sequences
        ...
        """
    seg_model_ckpt = torch.load(os.path.join(trained_models.__path__[0],'segmentation.pth'))
    tr_model_ckpt = torch.load(os.path.join(trained_models.__path__[0],'tracker.pth')) 
    print('Run segmentation process ...')
    label_img_list,new_img_list=[],[]
    for img in img_list:
        label_img,_,_,img_resized=apply_img_segmentation(img)
