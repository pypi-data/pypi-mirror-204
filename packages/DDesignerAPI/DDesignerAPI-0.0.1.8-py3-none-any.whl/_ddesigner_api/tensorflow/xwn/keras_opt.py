...
class Optimization:
    ...
    def optimize(self, x, transpose=False):
        '''
        x is AutoCastDistribuedVariable
        '''
        if transpose:
            _x = tf.tanspose(x, self.transpose_axis)
        else:
            _x = tf.identity(x)
        ...
        if transpose:
            _x = tf.tanspose(x_opt, self.transpose_axis)
        else:
            _x = tf.identity(x_opt)

        x = tf.compat.v1.assign(x, _x)
        return x
