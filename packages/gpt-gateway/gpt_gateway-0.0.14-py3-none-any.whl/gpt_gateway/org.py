...
class Org(OrgModel):
    ...
    def prompt_tasks(self, name=None, version=None) -> list[PromptTask]:
        ...
        return self.session.get(f"/orgs/{self.id}/prompt_tasks{query}").json()
    ...
    def create_prompt_task(self, 
        ...
        return PromptTask(self.session, **rsp.json())
    ...
    def get_prompt_task(self, prompt_task_id: int) -> PromptTask:
        return PromptTask(self.session, **self.session.get(f"/orgs/{self.id}/prompt_tasks/{prompt_task_id}").json())
