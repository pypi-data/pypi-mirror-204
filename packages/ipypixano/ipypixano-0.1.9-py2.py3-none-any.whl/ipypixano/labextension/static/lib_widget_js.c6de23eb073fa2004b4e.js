...
class PixanoModel extends base_1.DOMWidgetModel {
    defaults() {
        return Object.assign(Object.assign({}, super.defaults()), { _model_name: PixanoModel.model_name, _model_module: PixanoModel.model_module, _model_module_version: PixanoModel.model_module_version, _view_name: PixanoModel.view_name, _view_module: PixanoModel.view_module, _view_module_version: PixanoModel.view_module_version, element: 'pxn-rectangle', image_path: '', image_data: '', mode: 'none', img_count: 0, counter: 0, toggle_labels: 0, 
            ...
            //targetClass:1,
            //clsMap:{},
            maskVisuMode: 'INSTANCE', prediction: [], selectedCategory: '', label_schema: '', graph_setting: {}, image_screenshot: '', screenshot: '', do_screenshot: 0, client_ready: false });
    }
}
...
class PixanoView extends base_1.DOMWidgetView {
    ...
    render() {
        ...
        this.annotations = [];
        const link_for_material = document.createElement('p');
        ...
        this.pxnElement.classList.add("pixano-element");
        const image_in = this.model.get('image');
        //if (image_in=='') return
        ...
        this.properties_panel = document.createElement("div");
        this.properties_panel.classList.add('properties-panel');
        let label_schema_python = this.model.get('label_schema');
        let graph_setting = this.model.get("graph_setting");
        console.log("label_schema_python", label_schema_python, this.model.get('label_schema'));
        console.log("graph_setting", label_schema_python, this.model.get('graph_setting'));
        if (this.pxn_element_type === 'pxn-graph') {
            if (graph_setting.edges) {
                console.log("graph_setting.edges", graph_setting.edges);
                this.pxnElement.settings.edges = graph_setting.edges;
            }
            if (graph_setting.vertex_names) {
                console.log("graph_setting.vertex_names", graph_setting.vertex_names);
                this.pxnElement.settings.vertexNames = [...graph_setting.vertex_names];
            }
            console.log("pxn-graph", this.pxnElement.settings.edges, this.pxnElement.settings.vertexNames);
        }
        if (label_schema_python.category && label_schema_python.category.length > 0) {
            ...
            this.properties_panel.appendChild(this.attributePicker);
            main.appendChild(this.properties_panel);
        ...
        this._onAnnotationsInChanged();
        this.model.set("client_ready", true);
        this.pxnElement.addEventListener('load', () => {
            // refresh annoations on media loaded
            this.refreshPixano();
        });
        console.log("END RENDER");
        // this.pxnElement.renderer.addEventListener('resize', () => {
        //   // refresh annoations on media loaded
        //   this.resizePixano();
        // });
        //this.pxnElement.renderer.resize() ;
        //console.log("canvasElement",this.pxnElement.canvasElement())
    }
    
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    refreshPixano() {
        console.log("refreshPixano");
        const pxn_element_type = this.model.get('element');
        this.pxnElement.renderer.resizeWindow();
    ...
    resizePixano() {
        console.log("---------");
        console.log("resizePixano", this.pxnElement.renderer.canvasWidth, this.pxnElement.renderer.canvasHeight);
        console.log("resizePixano", this.pxnElement.renderer);
    }
    init_model_interaction() {
        ...
        this.model.on('change:label_schema', this._onSchemaChanged, this);
        if (this.pxn_element_type.search("segmentation") !== -1) {
    ...
    async onMessage(message, buffers) {
        ...
        if ("set_annotations" in message) {
            ...
            console.log("msg set_annotations");
            new_thing = true;
        }
        switch (pxn_element_type) {
            ...
            case 'pxn-smart-rectangle':
                let shapes;
                if (this.attributePicker) {
                    shapes = [...this.annotations].map(shape => {
                        var newShape = shape;
    ...
    _onAnnotationsInChanged() {
        ...
        const pxn_element_type = this.model.get('element');
        if (annotations_input[0] !== '-' && annotations_input.length > 1) {
            ...
            switch (pxn_element_type) {
                ...
                case 'pxn-smart-rectangle':
                    let shapes;
                    if (this.attributePicker) {
                        shapes = [...annotations_input].map(shape => {
                            var newShape = shape;
                            newShape.color = this.attributePicker._colorFor(newShape.category);
                            return newShape;
                        });
    ...
    onAttributeChanged() {
        ...
        const pxn_element_type = this.model.get('element');
        // si en mode creation on ne modifie pas les class avec l'attribute picker
        if (this.pxnElement.mode.search("create") !== -1) {
            return;
        }
        switch (pxn_element_type) {
            case 'pxn-classification':
            ...
            case 'pxn-polygon':
                this.selectedIds.forEach((id) => {
                    ...
                    console.log("shape.category.name", shape);
                    this.model.set("selectedCategory", shape.category);
...
//# sourceMappingURL=lib_widget_js.c6de23eb073fa2004b4e.js.map