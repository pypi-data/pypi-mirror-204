                ...
                for version in page.get("Versions", []):
                    ...
                    if len(objects_to_delete) == 50000:
                        ...
                        try:
                            ...
                            if 'Errors' in result:
                                objects_to_delete = [obj['Key'] for obj in result['Errors']]
                        except Exception as e:
                            print(f"Deletion failed with error: {str(e)}")
                            if len(objects_to_delete) > 1:
                                print("Trying to delete the next object...")
                                objects_to_delete.pop(0)
                                future = executor.submit(
                                    self.bucket.delete_objects,
                                    Delete={"Objects": objects_to_delete},
                                )
                                ...
                                print("Failed to delete object, moving to the next one...\n")
                                objects_to_delete = []
