...
def test_xe_frac(regtest):
    ...
    def print_stat(name, values):
        ...
        print("   min   is %.4f" % np.min(array), file=regtest)
        print("   max   is %.4f" % np.max(array), file=regtest)
        print("   mean  is %.4f" % np.mean(array), file=regtest)
        print("   std   is %.4f" % np.std(array), file=regtest)
