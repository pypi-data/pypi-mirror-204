...
def plot_classes(embedding, target):
    """ Plot umap_embeddings for HDBSCAN assignments
    :param embedding:
    :param target:
    """
    ...
    plt.scatter(embedding[:, 0], embedding[:, 1], s=0.1, c=target, cmap='Spectral', alpha=1.0)
    plt.setp(ax, xticks=[], yticks=[])
    cbar = plt.colorbar(boundaries=np.arange(len(np.unique(target))+1) - 0.5)
...
def plot_classes_scatter(data, assignments):
    """ Plot umap_embeddings for HDBSCAN assignments
    :param data: 2D array, umap_embeddings
    :param assignments: 1D array, HDBSCAN assignments
    """
    uk = list(np.unique(assignments))
#     uk = list(assignment.max())
#     R = np.linspace(0, 1, assignments.max())
    R = np.linspace(0, 1, len(uk))
    cmap = plt.cm.get_cmap("Spectral")(R)
    ...
    for g in np.unique(assignments):
        try:
            ...
            ax.scatter(umap_x[idx], umap_y[idx], c=cmap[g],
                       label=g, s=.5, marker='o', alpha=0.4)
        except:
            pass
    ax.set_xlabel('Dim. 1')
    ax.set_ylabel('Dim. 2')
