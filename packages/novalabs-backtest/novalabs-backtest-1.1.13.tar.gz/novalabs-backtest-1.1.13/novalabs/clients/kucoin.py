...
class Kucoin(ClientInterface):
    ...
    def _get_candles(
        ...
        _end_time = start_time + self.limit * _interval_ms
    ...
    def _get_earliest_timestamp(self, pair: str, interval: str) -> int:
        ...
        _interval_ms = interval_to_milliseconds(interval)
        min_date = int(datetime(2020, 1, 1).timestamp() * 1000)
        return max(int(current_time - (15 * _interval_ms * self.limit)), min_date)
    ...
    def get_historical_data(
        ...
        while True:
            ...
            temp_data = self._get_candles(
                pair=pair, interval=interval, start_time=start_time, end_time=end_ts
            )
