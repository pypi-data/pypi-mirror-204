...
def determine_clk_name_props(file_path: pathlib.Path) -> Dict[str, Any]:
    ...
    try:
        ...
        # The pandas stubs seem to be missing .diff() for SeriesGroupBy, so we disable type checking here
        sampling_rate = (
            clk_df.reset_index("J2000").groupby(level="CODE")["J2000"].diff().groupby(level="CODE").median().median()  # type: ignore
        )
...
def determine_snx_name_props(file_path: pathlib.Path) -> Dict[str, Any]:
    ...
    try:
        ...
        start_epoch = gn_datetime.round_timedelta(
            props_from_header_line["start_epoch"], datetime.timedelta(hours=1), abs_tol=datetime.timedelta(minutes=15)
        )
        end_epoch = gn_datetime.round_timedelta(
            props_from_header_line["end_epoch"], datetime.timedelta(hours=1), abs_tol=datetime.timedelta(minutes=15)
        )
        name_props["start_epoch"] = start_epoch
        name_props["end_epoch"] = end_epoch
        ...
        if "SOLUTION/EPOCHS" in snx_blocks:
            ...
            sampling_rate = name_props["timespan"]
        name_props["sampling_rate"] = sampling_rate
        logging.debug(f"name_props prior to adding props extracted from name = {name_props}")
        # If we extracted solution type from the name we keep it
        # Otherwise we can't easily have an opinion and so step aside for defaults
        # Likewise for version and project (which can't be found in file)
...
def determine_sp3_name_props(file_path: pathlib.Path) -> Dict[str, Any]:
    ...
    try:
        ...
        # The pandas stubs seem to be missing .diff() for SeriesGroupBy, so we disable type checking here
        sampling_rate = (
            sp3_df.reset_index(0, names="Epoch").groupby(level=0)["Epoch"].diff().groupby(level=0).median().median()  # type: ignore
        )
        name_props["start_epoch"] = start_epoch
        name_props["end_epoch"] = end_epoch
        name_props["timespan"] = timespan
