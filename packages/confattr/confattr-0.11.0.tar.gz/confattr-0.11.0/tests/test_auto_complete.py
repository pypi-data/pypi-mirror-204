...
def test__quote_path__tilde() -> None:
	cf = ConfigFile(appname='test')
	include = cf.command_dict['include']

	assert include.quote_path('~') == '~'
...
def test__quote_path__no_quote() -> None:
	...
	assert include.quote_path('a/b/c') == 'a/b/c'
...
def test__get_completions_for_file_name__exclude(monkeypatch: pytest.MonkeyPatch) -> None:
	...
	cf = ConfigFile(appname='test')
	include = cf.command_dict['include']
	start_of_line, completions, end_of_line = include.get_completions_for_file_name('a', relative_to='.', start_of_line='', end_of_line='', exclude=r'.*~$')
	assert start_of_line == ''
	assert end_of_line == ''
