...
class XTS(Broker):
    ...
    def orders(self, tag=None, order_type=None):
        ...
        orders.loc[:, "status"] = orders.status.replace(
            ["Rejected", "Cancelled", "Filled"], ["REJECTED", "CANCELLED", "COMPLETE"]
        )
    ...
    def positions(self):
        ...
        positions.loc[:, "exchange_symbol"] = (
            self.get_exchange_code(positions["exchange"])
            + ":"
            + positions["ExchangeInstrumentID"]
        )
        ...
        positions.loc[:, "option_type"] = np.where(
            "PE" == (positions.tradingsymbol.str.split(" ")[-2]), "PE", "CE"
        )

        positions.loc[:, "option_type"] = np.where(
            positions.exchange != "NFO", None, positions.option_type
        )
